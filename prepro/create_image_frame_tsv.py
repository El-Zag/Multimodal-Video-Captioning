...
def main(dataset="VATEX", tsv="train", image_size=-1, num_frames=32, num_workers=32):
    output_folder = f"datasets/{dataset}/frame_tsv"
    os.makedirs(output_folder, exist_ok=True)
    # To generate a tsv file:
    # data_path: path to raw video files
    global data_path
    if dataset == "MSRVTT":
        data_path = f"datasets/MSRVTT-v2/{num_frames}frames/" 
    else:
        data_path = f"datasets/{dataset}/{num_frames}frames/" 
    
    data = load_tsv_to_mem(f'datasets/{dataset}/{tsv}.img.tsv')
    ...
    def gen_rows():
        with mp.Pool(num_workers) as pool, tqdm(total=len(data)) as pbar:
            for idx, line_item in enumerate(
                ...
                yield(line_item)
    if image_size < 0:
        resolved_visual_file = f"{output_folder}/{tsv}_{num_frames}frames.img.tsv" 
    else:
        resolved_visual_file = f"{output_folder}/{tsv}_{num_frames}frames_img_size{image_size}.img.tsv"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", help="MSRVTT-v2/VATEX",
                        type=str, default="VATEX")
    parser.add_argument("--tsv", help="train/val/public_test",
    ...
    main(args.dataset, args.tsv, args.image_size, args.num_frames, args.num_workers)