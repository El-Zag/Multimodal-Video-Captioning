...
from src.evalcap.utils_caption_evaluate import evaluate_on_coco_caption, ScstRewardCriterion
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        #batch = tuple(batch[i].to(args.device) for i in range(6)) + ([audio_tensor.to(args.device) for audio_tensor in batch[6]],)
        ...
        if not args.scst:
            ...
            #assert inputs['audio_feat'].shape == torch.Size([6, 10, 512]), f"Audio feature shape: {inputs['audio_feat'].shape}"
            if iteration == 1:
                for k, v in inputs.items():
                    if isinstance(v, list) :
                        assert len(v) > 0
                        logger.info(f'{k} = {len(v)} x {v[0].shape}')
                    else:
        ...
        if args.learn_mask_enabled and not args.scst:
        ...
        if backward_now:
            global_step += 1
            #print(running_loss.val, global_step)
...
def scst_train_iter(args, train_dataloader, model, scst_criterion, 
    ...
    inputs = {'is_decode': True,
        'input_ids': batch[0], 'attention_mask': batch[1],
        'token_type_ids': batch[2], 'img_feats': batch[3],
        'masked_pos': batch[4], 'audio_feat':batch[5],
        'do_sample': False,
...
def check_arguments(args):
    ...
    #TO MODIFY ATTENTION MASK
    args.max_img_seq_length = int((args.max_num_frames/2)*(int(args.img_res)/32)*(int(args.img_res)/32)) + 10
    #args.max_img_seq_length = 10
...
def main(args):
    ...
    if args.do_train:
        ...
        args.save_steps = args.global_iters_per_epoch // 5
        #print(f"max_global_step : {args.max_global_step}, global_iters_per_epoch : {args.global_iters_per_epoch}, save_steps : {args.save_steps}")
