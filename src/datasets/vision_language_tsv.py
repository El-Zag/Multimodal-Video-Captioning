...
class VisionLanguageTSVDataset(object):
    ...
    def get_row_from_tsv(self, tsv, img_idx):
        ...
        if self.is_composite:
            ...
            assert row[0] == self.image_keys[img_idx], f"{self.get_valid_tsv()}, {tsv}, {img_idx}, {row[0]},{self.image_keys[img_idx]}"
        return row
    ...
    def __getitem__(self, idx):
        ...
        youtube_id = Path(img_key).stem
        
        assert os.path.isfile(f"{self.args.data_dir}/{self.args.audio_feat_dir}/{youtube_id}.pt")
        aud_feat = torch.load(f"{self.args.data_dir}/{self.args.audio_feat_dir}/{youtube_id}.pt")
        #aud_feat = torch.rand((10, 512), dtype=torch.float16)
